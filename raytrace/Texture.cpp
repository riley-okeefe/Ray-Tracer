...
cv::Vec3b Texture::texture_lookup_bilinear(const Vec2& uv) {
    ...
    //Get the texture colors of the four pixels    
    cv::Vec3b pixel00 = get_pixel(i0, j0);//iu0
    cv::Vec3b pixel01 = get_pixel(i0, j1);//iv1
    cv::Vec3b pixel10 = get_pixel(i1, j0);//iv0
    cv::Vec3b pixel11 = get_pixel(i1, j1);


    //Find the interpolated color
    cv::Vec3b pixelColor;    
    pixelColor[0] = au * av * pixel00(0) + au * bv * pixel01(0) + bu * av * pixel10(0) + bu * bv * pixel11(0);
    pixelColor[1] = au * av * pixel00(1) + au * bv * pixel01(1) + bu * av * pixel10(1) + bu * bv * pixel11(1);
    pixelColor[2] = au * av * pixel00(2) + au * bv * pixel01(2) + bu * av * pixel10(2) + bu * bv * pixel11(2);
